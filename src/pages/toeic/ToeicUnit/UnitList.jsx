...
import { deleteUnit, fetchUnitByBookId } from "../ToeicAPI";

const UnitList = ({openForm, bookId}) => {
    const queryClient = useQueryClient()

    const { data: units, isLoading, error } = useQuery(['units', bookId], () => fetchUnitByBookId(bookId))
    ...
    const rows = units.map((element) => (
        <Table.Tr key={element.id}>
            ...
            <Table.Td>{truncate(element.bookId, {length: 10})}</Table.Td>
